...
func (s *DialService) CreateDial(ctx context.Context, d *wtf.DialCreate) (*wtf.Dial, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, err
	}
	...
	if userID == 0 {
		return nil, wtf.Errorf(wtf.EUNAUTHORIZED, "You must be logged in to create a dial.")
	}
	dial := wtf.Dial{}
